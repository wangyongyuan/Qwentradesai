...
15m K线数据更新
    ↓
KlineSyncManager._fetch_and_save_klines()
    ↓
保存K线数据到数据库
    ↓
更新技术指标（IndicatorCalculator）
    ↓
调用 market_detector.detect()
    ↓
MarketDetector.detect() 开始检测
```
...
2. 环境层过滤 (_filter_layer)
   ...
   └── 判断多时间框架对齐
   ↓
   如果 market_mode == NEUTRAL → 直接返回，不生成信号
   如果 market_active == False → 直接返回，不生成信号
...
3. 触发层检测
   ...
   └── 检查价格形态（吞没形态）
   ↓
   如果没有任何触发条件 → 直接返回，不生成信号

4. 确认层验证 (_confirmation_layer)
   ├── 成交量确认（动态阈值）
   └── 布林带确认（价格突破）
   ↓
   如果确认层不通过 → 直接返回，不生成信号
...
6. 保存检测快照
   └── 插入 market_detection_snapshots 表
...
**检测快照（market_detection_snapshots）：**
- 每次检测都会保存快照，无论是否生成信号
- 记录所有检测结果，包括环境层、触发层、确认层的所有状态
- 用于回溯分析和参数优化

**交易信号（market_signals）：**
- 只有通过所有三层过滤后才会生成信号
- 记录信号类型、强度、置信度、触发因子等
- 包含信号有效期，过期后自动标记为EXPIRED
...
SELECT 
    ...
    signal_strength,
    confidence_score
FROM market_detection_snapshots
WHERE symbol = 'ETH'
...
LEFT JOIN market_detection_snapshots snap ON s.snapshot_id = snap.id
WHERE s.symbol = 'ETH'
...
-- 提高RSI做多上限，允许在更高RSI时做多
UPDATE system_config 
SET config_value = '85.0' 
WHERE config_key = 'DETECTOR_RSI_LONG_THRESHOLD';
```

**方法2：通过API修改（如果实现了配置API）**

```python
# 未来可以添加配置API
PUT /api/config/DETECTOR_VOLUME_STD_MULTIPLIER
{
    "value": "1.0"
}
```

**注意：** 参数修改后，下次检测时会自动使用新参数，无需重启服务。
...
目前市场检测器仅对ETH运行，如需扩展到其他币种：

**1. 修改 `app/main.py`**

```python
# 将 detector_symbols 扩展为包含其他币种
detector_symbols = ['ETH', 'BTC']  # 添加BTC
```
...
1. **市场处于NEUTRAL模式**
   - 价格在EMA55附近震荡，无法确定趋势方向
   - 解决方案：这是正常行为，NEUTRAL模式下不生成信号

2. **市场不活跃**
   - 布林带宽度低于阈值，市场波动太小
   - 解决方案：降低 `DETECTOR_BB_WIDTH_THRESHOLD` 参数

3. **触发条件不满足**
   - 没有任何触发维度满足条件
   - 解决方案：检查触发层的各个维度，可能需要调整RSI阈值

4. **确认层未通过**
   - 成交量或布林带确认失败
   - 解决方案：降低 `DETECTOR_VOLUME_STD_MULTIPLIER` 参数
...
-- 查看哪些触发因子最常出现
SELECT 
    trigger_factors->>'momentum_turn' as momentum_turn,
    trigger_factors->>'ema_cross' as ema_cross,
    trigger_factors->>'rsi_extreme' as rsi_extreme,
    COUNT(*) as count
FROM market_signals
WHERE symbol = 'ETH'
  AND detected_at > NOW() - INTERVAL '7 days'
GROUP BY 
    trigger_factors->>'momentum_turn',
    trigger_factors->>'ema_cross',
    trigger_factors->>'rsi_extreme'
ORDER BY count DESC;
```
...
```bash
# 查看系统日志
tail -f logs/qwentradeai.log | grep "市场检测"
```
...
SELECT 
    COUNT(*) as total,
    COUNT(ema_55) as has_ema55,
    COUNT(rsi) as has_rsi,
    COUNT(macd_histogram) as has_macd
FROM klines_15m
WHERE symbol = 'ETH'
...
市场检测器采用**层级过滤模型**，通过三层串行过滤确保信号质量：

1. **环境层**：判断市场方向和活跃度，过滤不适合交易的市场状态
2. **触发层**：在确定的方向上寻找具体的入场时机
3. **确认层**：验证信号的真实性，确保是真金白银的突破
...
1. **动态配置**：所有参数存储在数据库中，支持运行时调整
2. **完整记录**：每次检测都保存快照，支持回溯分析
3. **信号分级**：根据触发条件数量计算信号强度和置信度
4. **自动过期**：信号有有效期，过期后自动标记
5. **多时间框架**：支持15m、4h、1d多时间框架共振

#### 6.5.3 使用建议

1. **参数调整**：采用渐进式调整，每次只调整一个参数
2. **监控分析**：定期查看检测快照和信号统计，分析信号质量
3. **回测验证**：重要参数调整前，建议使用历史数据回测
4. **平衡优化**：在信号数量和信号质量之间找到平衡点

#### 6.5.4 未来扩展方向

1. **更多触发维度**：可以添加更多技术指标作为触发条件
2. **机器学习优化**：使用历史数据训练模型，优化参数配置
3. **多币种支持**：扩展到更多币种，支持多币种同时检测
4. **信号回测**：添加信号回测功能，评估信号质量
5. **实时监控**：添加Web界面，实时监控检测状态和信号生成

---

**文档完成。** 本文档完整描述了市场检测器模块的设计理念、架构、实现细节、配置参数、数据流和使用说明。如有疑问，请参考代码实现或联系开发团队。
